...
function M.load_settings(settings)
	...
	if settings.punch_legacy ~= legacy then
		release()
	end
	use_punch = settings.use_punch
	legacy = settings.punch_legacy
end
...
function M.imgui_config_drawer()
	ImGui.Text(string.format("Punch Config (fov binding: %s, mode: %s)",
		has_fov_binding and "yes" or "no", legacy and "legacy console/PiP" or "effector hip/ads"))
