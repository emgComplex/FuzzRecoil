...
function M:clamp_angle()
	if not m_settings.use_cam_max_angle then
		return
	end
	if self.max_angle > 0 and self.angle > self.max_angle then
		self.angle = self.max_angle
	end
end

function M:update_cubic(dt)
	if math.abs(self.vel) <= 0.01 then
		return
	end
...
function M:update_exp(dt)
	...
	--auto fire impulse decay and step divisor from config, tunable in imgui
	local decay = math.exp(-dt * m_cfg.cam_impulse_decay)
	local step = self.vel * (1 - decay) / m_cfg.cam_step_div
